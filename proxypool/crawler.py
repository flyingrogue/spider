...
class Crawler(object,metaclass=ProxyMetaclass):
    ...
    def crawl_xundaili(self):
        url='http://api.xdaili.cn/xdaili-api//privateProxy/getDynamicIP/DD20189278850IlWvrI/f77e2395108111e79ff07cd30abda612?returnType=2'
        html=self.get_page(url)
        if html:
            result=json.loads(html)
            proxy=result.get('RESULT')
            yield proxy.get('wanIp')+':'+proxy.get('proxyport')

    #获取芝麻代理
    # def crawl_zhimadaili(self):
    #     url='http://webapi.http.zhimacangku.com/getip?num=20&type=2&pro=&city=0&yys=0&port=1&pack=30704&ts=0&ys=0&cs=0&lb=1&sb=0&pb=45&mr=1&regions='
    #     html=self.get_page(url)
    #     if html:
    #         proxies=json.loads(html).get('data')
    #         for proxy in proxies:
    #             yield str(proxy.get('ip'))+':'+str(proxy.get('port'))

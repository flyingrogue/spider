...
def main(year,mon,mysql):
    for shibor_item in Shibor(year, mon):
        if shibor_item:
            ...
            if bond_item:
                #将'Date'字段转换为date格式
                temp=shibor_item['Date'].split('-')
                bond_item['Date']=date(int(temp[0]),int(temp[1]),int(temp[2]))
                print(bond_item)
                #将item插入mysql
                mysql.insert('中国国债到期收益率', bond_item)
            time.sleep(2)

if __name__=='__main__':
    while True:
        now=datetime.now()
        if now.hour in [17,] and now.minute in range(35,50,3):
            mysql=Mysql()

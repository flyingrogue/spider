...
class Distance():
    ...
    def main(self):
        ...
        sql3 = 'select CITY,LONGITUDE,LATITUDE,S_CAPITALOPERATION_PCT,VOTING_RIGHTS,S_CAPITALOPERATION_AMOUNT,S_CAPITALOPERATION_COREGCAP' \
               ' from ASHARECOMPANYHOLDSHARES where S_INFO_WINDCODE like "{}.%" and ADDRESS is not null and ANN_DT regexp "^2018" and' \
               '(S_CAPITALOPERATION_AMOUNT>=1000 or S_CAPITALOPERATION_COREGCAP>=1000) and (S_CAPITALOPERATION_PCT>=50 or VOTING_RIGHTS>=50)'
        sql4 = 'SELECT S_SEGMENT_ITEM,PCT_SEGMENT_SALES FROM ASHARESALESSEGMENT where S_INFO_WINDCODE like "{}.%"' \
               ...
               '北京|天津|上海|重庆|河北|山西|内蒙古|辽宁|吉林|黑龙江|江苏|浙江|安徽|福建|江西|山东|河南|湖北|湖南|广东|广西|海南|四川|贵州|云南|西藏|陕西|甘肃|青海|宁夏|新疆"'
        ...
        for code in codes:
            ...
            if compid:
                ...
                if temp:
                    ...
                    #从wind中获取上市公司的子公司信息
                    self.cursor2.execute(sql3.format(code))
                    ...
                    #如果wind中没有含地址的子公司信息，则根据wind的分地区(只选取细分到省或市的)主营业务表得到各地的权重
                        ...
                        self.cursor3.execute(sql4.format(code))
                        ...
                        #若分地区主营业务表也无有效信息，则直接用注册城市，权重为100%
                            ...
                            #print(code, '此公司无子公司地址使用注册地址', city)
                            count2+=1
                else:
                    index.append((code,0))
                    postion.append(None)
        ...
        # np.save('/home/jbliu/projects/distance.npy',np.array(data))
        # np.save('/home/jbliu/projects/postion.npy',np.array(postion))
        #将索引列表中的元组定义为自定义数组类型，这样才可将索引列表转换为一维数组
        # t=np.dtype([('code',np.str,20),('wgt',np.float)])
        # np.save('/home/jbliu/projects/index.npy',np.array(index,dtype=t))

...
class CookiesMiddleware():
    def __init__(self,cookies_url,cookies):
...
class AbuyunMiddleware(object):
    ...
    def from_crawler(cls, crawler):
        return cls(
            proxyUser=crawler.settings.get('PROXYUSER'),proxyPass=crawler.settings.get('PROXYPASS'),proxyServer=crawler.settings.get('PROXYSERVER')
        )

    def process_request(self, request, spider):
        request.meta["proxy"] = self.proxyServer
        request.headers["Proxy-Authorization"] = self.proxyAuth
        self.logger.debug('使用代理' + self.proxyServer)